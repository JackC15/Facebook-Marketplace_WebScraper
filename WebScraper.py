...
plt.savefig(f'{make} {model} Year vs Price.png')
plt.show()
...
plt.xticks(yearly_mileage.index, rotation=45)
plt.bar(yearly_mileage.index, yearly_mileage.values)
plt.savefig(f'{make} {model} Year vs Mileage.png')
plt.show()
...
plt.savefig(f'{make} {model} Year vs Listings.png')
plt.show()

cheapest_vehicle_url = filtered_df[filtered_df["Price"] == filtered_df["Price"].min()]["URL"].values[0]
print(cheapest_vehicle_url)
